...
```
 ...
 To see this new alias property, use a [Get-Member](http://go.microsoft.com/fwlink/?LinkId=113322) command on any array, as shown in the following example.
...
-   To make your extended types file available to the current session, use the [Update-TypeData](http://go.microsoft.com/fwlink/?LinkId=113421) cmdlet to add the new file. If you want your types to take precedence over the types that are defined in other types files (including the Types.ps1xml file), use the `PrependData` parameter of the [Update-TypeData](http://go.microsoft.com/fwlink/?LinkId=113421) cmdlet.
 ...
 To make your extended types file available to all future sessions, add the types file to a module, export the current session, or add the [Update-TypeData](http://go.microsoft.com/fwlink/?LinkId=113421) command to your Windows PowerShell profile.
...
## Signing Types Files
 ...
 Types files should be digitally signed to prevent tampering because the XML can include script blocks. For more information about adding digital signatures, see [about_Signing](http://go.microsoft.com/fwlink/?LinkId=113268)
