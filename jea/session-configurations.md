...
No PowerShell providers are available, nor are any external programs (cmd.exe programs).
...
When one or more security groups is specified, the virtual account will not belong to the local or domain administrators group.
...
- It is harder to trace back actions to a user when using a gMSA account since every user shares the same run-as identity. You will need to consult PowerShell session transcripts to correlate users with their actions.
