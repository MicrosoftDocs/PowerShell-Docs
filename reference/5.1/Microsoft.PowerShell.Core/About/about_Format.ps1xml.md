...
1. **Certificate.Format.ps1xml**
   1. Objects in the Certificate store, such as X.509 certificates and
    certificate stores.    
1. **DotNetTypes.Format.ps1xml**
   1. Other .NET Framework types, such as CultureInfo, FileVersionInfo,
    and EventLogEntry objects.
1. **FileSystem.Format.ps1xml**
   1. File system objects, such as files and directories.
1. **Help.Format.ps1xml**
   1. Help views, such as detailed and full views, parameters, and
    examples.
1. **PowerShellCore.format.ps1xml**
   1. Objects generated by Windows PowerShell core cmdlets, such as Get-Member 
    and Get-History.
1. **PowerShellTrace.format.ps1xml**
   1. Trace objects, such as those generated by the Trace-Command cmdlet.
1. **Registry.format.ps1xml**
   1. Registry objects, such as keys and entries.
...
\<View> tags that define each view. 

A typical \<View> tag includes the following tags:
