...
An array is a data structure that is designed to store a collection of
...
(..). For example, to create and initialize an array named "$B" that
contains the values 5 through 8, type:
...
object array (type: System.Object[]). To determine the data type of an
array, use the GetType() method. For example, to determine the data type of
the $a array, type:
...
with an array type enclosed in brackets. For example, to create a 32-bit
integer array named $ia containing four integers (1500, 2230, 3350, and
4000), type:
...
The array sub-expression operator creates an array, even if it contains
zero or one object.
...
elements in the array, type the array name. For example, assuming *$a* is
an array containing integers 0, 1, 2, until 9; typing:
...
position 0. Enclose the index number in brackets. For example, to display
the first element in the *$a* array, type:
...
Negative numbers count from the end of the array. For example, "-1" refers
to the last element of the array. To display the last three elements of the
...
Also, one common mistake is to assume *$a[0..-2]* refers to all the
elements of the array, except for the last one. It refers to the first,
last, and second-to-last elements in the array.
...
Removes all elements in the array. The following example shows the effect
of the clear method.
...
Allows to iterate over all elements in the array and execute the given
script for each element of the array.
...
`$null` for the element to show after the `Where`
...
one at a time and Get-Member returns the type of each item in the array
(ignoring duplicates).
...
second item in an array of arrays. Windows PowerShell pipes the arrays one
at a time and Get-Member returns the members of the array. Like the next
two examples.
