...
`Get-WinEvent` lists event logs and event log providers. To interrupt the command, press `CTRL + C`.
You can get events from selected logs or from logs generated by selected event providers. And, you
can combine events from multiple sources in a single command. `Get-WinEvent` allows you to filter
...
see [about_Automatic_Variables](../Microsoft.PowerShell.Core/about/about_automatic_variables).
...
parameter accepts only one value. For more information, see [about_Foreach](../Microsoft.PowerShell.Core/about/about_Foreach).

```PowerShell
$S = 'Server01', 'Server02', 'Server03'
ForEach ($Server in $S) { Get-WinEvent -ListLog Application -ComputerName $Server; $Server }
```

```Output
LogMode   MaximumSizeInBytes RecordCount LogName
-------   ------------------ ----------- -------
Circular            15532032       14577 Application
Server01
Circular            15532032       9689  Application
Server02
Circular            15532032       5309  Application
Server03
```
...
curly braces (`{ }`) runs the `Get-WinEvent` command and displays the server name. The **ListLog**
parameter specifies the **Application** log. The **ComputerName** parameter uses the variable
`$Server` to get log information from each server.
