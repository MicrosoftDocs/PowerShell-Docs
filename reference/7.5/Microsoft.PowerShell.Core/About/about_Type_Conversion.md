...
Numeric types can be freely converted to one another as long as the target type
...
PowerShell operators don't guarantee that the result of an expression is of the
same type as its operands. PowerShell can convert the operands in an expression
to produce a reasonable result. Also, some operators have type-specific
behaviors.
