...
Register-ScheduledJob [-Name] <String> [-ScriptBlock] <ScriptBlock> [-ArgumentList <Object[]>]
 ...
 [-MaxResultCount <Int32>] [-RunAs32] [-RunNow] [-ScheduledJobOption <ScheduledJobOptions>]
 [-Trigger <ScheduledJobTrigger[]>] [-Confirm] [-WhatIf]
```
...
The subdirectory is named for the scheduled job and contains  an XML file for the scheduled job and records of its execution history.
...
However, if the job fails when Task Scheduler tries to run it, the error is not available to Windows PowerShell.
