...
Specifies the system scope where help is to be updated. Updating help at the **AllUsers** scope
requires administrative privileges on Windows systems. The `-Scope` parameter was introduced in
PowerShell Core version 6.1.

**CurrentUser** is the default scope for help for PowerShell 6.1 and above. **AllUsers** can be
