...
the substrings are returned. A value of 0 return all the
substrings.
...
If your input for max-substrings is a negative number then the splitting
processes starts at the end of the string. If there are more substrings, they
are concatenated to the final substring. If there are fewer substrings, all
the substrings are returned. A value of 0 returns all the substrings.
