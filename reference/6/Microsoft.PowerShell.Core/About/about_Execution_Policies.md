...
For non-Windows computers, the default execution policy is **Unrestricted**
and cannot be changed. The `Set-ExecutionPolicy` cmdlet is available but
displays a PowerShell console message that it is not supported.
...
are downloaded from the Internet which includes
...
- Unsigned scripts can run. There is a risk of running malicious
scripts.
...
To change the PowerShell execution policy on your Windows
...
the execution policy to Undefined.
...
execution policy is Restricted, which is the default for Windows clients.

## Set a Different Execution Policy for One Session

You can use the **ExecutionPolicy** parameter of **pwsh.exe** to
...
at the command line, such as **Cmd.exe** or from PowerShell, and then use
the **ExecutionPolicy** parameter of **pwsh.exe** to set the execution
policy.
...
downloaded from the Internet which includes e-mail and instant
messaging programs.

You can sign the script or elect to run an unsigned script
without changing the execution policy.

Beginning in PowerShell 3.0, you can use the **Stream**
