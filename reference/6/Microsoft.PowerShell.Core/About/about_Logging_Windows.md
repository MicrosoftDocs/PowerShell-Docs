...
before logged events can appear in the event log.  For PowerShell,
this task is accomplished by running the `RegisterManifest.ps1` from
...
Registering an event provider places a lock in the binary having the data
