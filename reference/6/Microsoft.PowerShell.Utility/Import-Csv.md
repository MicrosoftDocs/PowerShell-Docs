...
The third command uses a pipeline operator to pipe the imported objects to the `Get-Member` cmdlet. The result shows that they are **System.Management.Automation.PSCustomObject** objects, not the **System.Diagnostic.Process** objects that `Get-Process` returns.

Because there is no entry type in the formatting files for the CSV version of the process objects, these objects are not formatted in the same way that standard process objects are formatted.
To display the objects, use the formatting cmdlets, such as `Format-Table` and `Format-List`.
...
$Header = "State", "MoreData", "StatusMessage", "Location", "Command", "StateInfo", "Finished", "InstanceId", "Id", "Name", "ChildJobs", "PSBeginTime", "PSEndTime", "PSJobTypeName", "Output", "Error", "Progress", "Verbose", "Debug", "Warning", "Information"
...
The next three commands use the `$A` variable to delete the original header (the first line) from the Jobs.csv file. Then, the `$A` variable redirects the data to the Jobs.csv file.
...
[!NOTE]
To create your Links.csv file, use the values shown in the example's `Get-Content` output.

The first command uses the `Get-Content` cmdlet to display the Links.csv file in the current directory.
...
The `Where-Object` cmdlet uses the new simplified command format that does not require symbols, script blocks, or curly braces.
...
`Import-Csv` substitutes a default name for the missing header row that becomes the property name of the object that `Import-Csv` returns.

[!NOTE]
To create your Projects.csv file, use the values shown in the example's `Get-Content` output.

The first command uses the `Get-Content` cmdlet to display the Projects.csv file in the current directory. 
The output shows that the header row is missing a value between **ProjectName** and **Completed**.
...
The third command uses a method to get the **H1** property value from the object created by `Import-Csv`.
...
The default is based on the culture settings of the Operating System. For example, the **en-US** culture will return a comma (,) by default.
