...
A pipeline object (|) sends the date to the `ConvertTo-Csv` cmdlet, which converts the date object to a series of CSV strings.
...
PSBeginTime   : 12/17/2018 16:15:52
PSEndTime     :
...
The third command deletes the original header from the CSV strings and returns it to the `$J` variable.
...
The resulting object has the **MoreData** and **StateInfo** properties, as specified by the header.
