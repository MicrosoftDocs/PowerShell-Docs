...
This cmdlet requires PowerShell remoting only when you use the *AsJob* parameter in a command.
...
This command restarts the Server01 remote computer and then waits up to 5 minutes (300 seconds) for PowerShell to be available on the restarted computer before it continues.
...
- Default.
Waits for PowerShell to restart.
- PowerShell.
Can run commands in a PowerShell remote session on the computer.
- WMI.
