...
Console pane is an **ISEEditor** object. Each [ISEFile][04] object has an associated **ISEEditor**
object. The following sections list the methods and properties of an **ISEEditor** object.
...
`$true`, which occurs when the caret is immediately before an opening parenthesis, bracket, or brace
- `(`,`[`,`{` - or immediately after a closing parenthesis, bracket, or brace - `)`,`]`,`}`. The
caret is placed before an opening character or after a closing character. If the **CanGoToMatch**
property is `$false`, then this method does nothing.
...
See the [Scripting Example][01] later in this topic.
...
after the closing character of a pair, then this property value is `$true`. Otherwise, it's
`$false`.
...
[05]: The-PowerShellTab-Object.md
