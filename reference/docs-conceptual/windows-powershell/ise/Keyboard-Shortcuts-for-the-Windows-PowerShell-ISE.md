...
| **Undo**                         | <kbd>CTRL</kbd>+<kbd>Z</kbd>                  | Script Pane, Command Pane  ** A bug in PowerShell ISE means that most of the time **Ctrl Z** won't jump the cursor to the last edited position. If you have scrolled since the change it can be quite time consuming to work out what you are undoing - making it almost impossible to go back far. It works successfully only when using a new tab - but never works after opening a file.  The present solution is therefore to open your file, mouse click within the script area,  **Ctrl-A** to select All, **Ctrl-C** to copy, **Ctrl-F4** to close the tab, **Ctrl-N** to open new tab, mouse- click in the empty script, **Ctrl-V** to paste, **Ctrl-S** to save and select the file name you just copied - save on top of that file. You can now continue developing and testing your script and the cursor/caret will now successfully jump to the changed text each time you press **Ctrl-Z** to undo or **Ctrl-Y** to redo                                                |
