...
PowerShell has the following new language elements in PowerShell:
...
`Import-DscResource` is a true dynamic keyword. PowerShell parses the specified module's root
module, searching for classes that contain the **DscResource** attribute.

### ImplementingAssembly

**ImplementingAssembly** is a part of **ModuleInfo**. It is set to the dynamic assembly created for
a script module if the script defines classes, or the loaded assembly for binary modules. It is not
set when **ModuleType** is **Manifest**.
...
`Static` is supported. It works like an attribute, as do the type constraints. It can be specified
in any order.
...
constructor (like the C\# syntax ": `this()`"). The workaround is to define a common `Init()`
method.

#### Creating instances

> [!NOTE]
> `New-Object` does not work with classes defined in PowerShell. Also, the type
...
`Hidden` can be applied to properties and methods (including constructors).
