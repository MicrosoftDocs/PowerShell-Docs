...
| RefreshFrequencyMins| Uint32| The time interval, in minutes, at which the LCM checks a pull service to get updated configurations and checks local configuration for drift. This value is ignored if the LCM is not configured in pull mode. The default value is 30.|
...
> [!NOTE]
> The originally intent in Pull scenarios was that the `RefreshFrequencyMins` is set to a longer
