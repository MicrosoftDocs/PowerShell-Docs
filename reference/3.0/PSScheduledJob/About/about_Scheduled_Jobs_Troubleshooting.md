...
```powershell
dir $home\AppData\Local\Microsoft\Windows\PowerShell
...
```powershell
Get-ScheduledJob ProcessJob | Get-JobTrigger | Format-Table ID, Enabled -AutoSize
```
...
```powershell
Get-WinEvent -LogName Microsoft-Windows-TaskScheduler/Operational | Where-Object {$_.Message -like "fail"}
```
