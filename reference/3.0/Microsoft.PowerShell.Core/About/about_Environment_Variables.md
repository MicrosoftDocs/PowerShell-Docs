...
is the dictionary key. The value of the variable is the dictionary value.
...
For example, to append ";C:\Temp" to the value of the Path
...
`Copy-Item` to change the values of environment variables.
For example, to use the `Set-Item` cmdlet to append ";C:\Temp"
to the value of the Path environment variable, use the following syntax:
...
Windows PowerShell session, add the change to your Windows PowerShell profile.
...
For more information about preference variables, see [about_Preference_Variables](about_Preference_Variables.md).
...
1. Use PowerShell.exe to start a session at the command line and
use its ExecutionPolicy parameter to set the execution policy for
the session.

2. Use the `Set-ExecutionPolicy` cmdlet. Use the **Scope** parameter with
a value of "Process".
...
see [Environment Methods](https://go.microsoft.com/fwlink/?LinkId=242783) in the MSDN library.
