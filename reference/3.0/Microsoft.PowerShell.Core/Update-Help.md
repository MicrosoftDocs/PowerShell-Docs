...
To run this command, start PowerShell with the "Run as administrator" option (`Start-Process PowerShell -Verb RunAs`).
...
Because these modules are in the `$pshome\Modules` directory, to run this command, start PowerShell with the "Run as administrator" option.
...
To run the `Register-ScheduledJob` command, start PowerShell with the "Run as administrator" option.
...
Invoke-Command -ComputerName (Get-Content Servers.txt) -ScriptBlock {Update-Help -SourcePath \\Server01\Share\Help -Credential Domain01\Admin01}
```
