...
online version: https://learn.microsoft.com/powershell/module/microsoft.powershell.core/about/about_execution_policies?view=powershell-7.7&WT.mc_id=ps-gethelp
schema: 2.0.0
...
behavior really matches **Bypass** because those platforms don't implement the
...
- `Unrestricted`
  ...
  - The default execution policy for non-Windows computers and can't be
    changed.
...
- `CurrentUser` - Second highest precedence. The setting is stored in the
  user-specific `powershell.config.json` file.
...
policy is set. You can't change the policy by editing the variable value.

During the session, the execution policy that's set for the session takes
precedence over an execution policy that's set in the configuration file for
...
The `PowerShellCoreExecutionPolicy.adm` and
`PowerShellCoreExecutionPolicy.admx` files add the **Turn on Script Execution**
policy to the Computer Configuration and User Configuration nodes in Group
Policy Editor in the following path:
...
For more information, see [about_Signing][06], [Get-Item][07], and
[Unblock-File][10].
...
[10]: xref:Microsoft.PowerShell.Utility.Unblock-File
