...
ms.date: 08/31/2026
online version: https://learn.microsoft.com/powershell/module/microsoft.powershell.security/set-executionpolicy?view=powershell-7.7&WT.mc_id=ps-gethelp
schema: 2.0.0
...
- `Process` - Highest precedence. The `Process` scope only affects the current PowerShell session.
  ...
  deleted.
- `CurrentUser` - Second highest precedence. The setting is stored in the user-specific
 `powershell.config.json` file.
- `LocalMachine` - Lowest precedence. The setting is stored in the `$PSHOME/powershell.config.json`
  file.

For more information about the `powershell.config.json` file, see
[about_PowerShell_Config](../Microsoft.PowerShell.Core/About/about_PowerShell_Config.md).
