...
## DESCRIPTION
`Test-Path` determines whether all elements of the path exist.
...
The asterisk at the end of the path indicates the contents of the `Commercial Buildings` directory.
...
Enter a `DateTime` object, such as one that [Get-Date](../Microsoft.PowerShell.Utility/Get-Date.md) returns, or a string that can be converted to a `DateTime` object, such as "August 10, 2011 2:00 PM".

Specify a time as a `DateTime` object.

`OlderThan` is a dynamic parameter that's only available within the FileSystem provider.
...
`NewerThan` is a dynamic parameter that's only available within the FileSystem provider.
It was introduced in Windows PowerShell 3.0.
...
Type a username, such as `User01` or `Domain01\User01`.
Or, enter a `PSCredential` object, such as one generated by the `Get-Credential` cmdlet.
If you type a username, this cmdlet prompts you for a password.
