...
### \-Hidden  
 Gets only hidden files and directories \(folders\).  By [default](default.md), [Get-ChildItem](../Microsoft.Powershell.Management/Get-ChildItem.md) gets only non\-hidden items, but you can use the Force parameter to include hidden items in the results.  
...
### \-Force  
 Gets hidden files and folders. By [default](default.md), hidden files and folder are excluded. You can also get hidden files and folders by using the Hidden parameter or the Hidden value of the Attributes parameter.  
