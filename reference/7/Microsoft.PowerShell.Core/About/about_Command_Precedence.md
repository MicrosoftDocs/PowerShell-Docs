...
PowerShell executes a file that has a literal match, before a wildcard match.
...
When you run `[a1].ps1`, the file `[a1].ps1` is executed even though the file
`a.ps1` is a wildcard match. This behavior is new to PowerShell v7.
...
Now let's delete the `[a1].ps1` file and attempt to run it again.

```powershell
Remove-Item -LiteralPath `C:\temp\test\[a1].ps1
C:\temp\test\[a1].ps1
```
...
You can see from the output that `a.ps1` runs this time because the wildcard
